...
with st.echo(code_location='below'):
    ...
    st.markdown("Пожалуйста, подождите некоторое время, подвисание может происходить из-за загрузки карт с многочисленными метками, но спустя несколько секунд всё будет работать исправно!")
    ...
    '''
Обработка данных с помощью pandas. Чтение из csv-файлов, сортировка по столбцу, удаление столбцов, обрезка строк, выбор строк с определенными значениями в определенных столбцах, вынимание значение из ячеек, изменение значений в ячейках при условиях, удаление строк, переиндексирование, изменение типов столбцов (в том числе to_numeric приведение к численному виду), изменение названий столбцов, удаление строк с NaN.
Веб-скреппинг. Осуществлялся в отдельно приложенном ipynb-файле. Использовались библиотеки Wikipedia и Selenium. С помощью Wikipedia производим поиск по имени и фамилии автора в википедии и переходим по ссылке из первого запроса к нужной статье. C помощью Selenium находим на странице основное фото автора и заносим ссылку на изображение в нашу таблицу. (Если у какой-то книги было несколько авторов, операция проделывалась для всех авторов.)
Работа с REST API (XML/JSON). Использовалось для нахождения географических координат книжных магазинов выбранной пользователем сети по заданным адресам магазинов.
Визуализация данных – 2 карты, изменяющихся в зависимости от выборов пользователей с отметками и всплывающим при нажатии текстом, граф с цветными пронумерованными вершинами, гистограмма (2 штуки в зависимости от выбора пользователя), коррелограмм.
Математические возможности Python (содержательное использование numpy/scipy, SymPy и т.д. для решения математических задач).  Преобразования numpy-массива для создания матрицы смежности, по которой в дальнейшем строится двудольный граф.
Streamlit. Размещено на Streamlit.
SQL. Обработка таблицы, сортировка по значениям определенного столбца и получение двух новых из нее в отдельном приложенном ipynb-файле.
Регулярные выражения. Использовались дважды. Первый раз для разделения описания книги на слова (это оправдано, т.к. недостаточно разделять строку используя пробел как разделитель, а перечислять всевозможные знаки препинания нецелесообразно). Второй раз для преобразования адресов книжных магазинов к нужному нам виду путем извлечения с помощью регулярных выражений нужных фрагментов строки (в этом случае обойтись без регулярных выражений реально, но регулярные выражения помогли существенно сократить код).
Работа с геоданными с помощью geopandas, shapely, folium и т.д.- использовался folium. Первая карта – карта Москвы с отмеченными магазинами книжной сети, выбранной пользователем. При кликании на метку магазина всплывает его адрес. Метки поставлены по полученным из адреса с помощью REST API долготе и широте. Вторая карта – карта мира с отмеченными 5 городами, которые изменяются в зависимости от выбора пользователя. При кликании на метку появляется название города. Метки поставлены по взятым из таблицы широте и долготе.
Машинное обучение- линейная регрессия для предсказания цены книги по ее рейтингу и версии (электронная версия/ мягкая обложка/ твердая обложка).
Работа с графами (библиотека networkx). Построение двудольного графа по заданной матрице смежности, нумерация вершин и окрашивания вершин из разных долей в разные цвета.
Дополнительные технологии (библиотеки, не обсуждавшиеся в ходе курса — например, телеграм-боты, нейросети или ещё что-нибудь) – библиотека Wikipedia для поиска словосочетания в поисковой строке википедии и нахождения ссылки первого запроса в поиске.
Объём (осмысленных строк кода, не считая import). – более 120 строк в основном файле (вместе с импортами и комментариями более 260) + около 40 строк в двух других приложенных файлах.
Целостность и общее впечатление судить не мне😊
    '''
